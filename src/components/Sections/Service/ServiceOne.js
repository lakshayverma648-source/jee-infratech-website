...
const ServiceOne = ({ className, data, start, limit }) => {
    return (
        <section className={`section services sm:pt-25 pt-15 pb-8 ${classNames(className)}`}>
            <div className="container flex flex-col items-center">

                <span className="section_tag tag animate">
                    What We Do?
                </span>
                ...
                <p className="section_desc lg:mt-5 mt-3 lg:w-1/2 body2 text-variant1 text-center animate">
                    We pride ourselves on delivering consistent, high-quality services that you can depend on. Whether it’s a small fix or a large-scale renovation
                </p>

                <ul className="services_list grid xl:grid-cols-3 md:grid-cols-2 lg:gap-7.5 gap-6 lg:mt-12 mt-9">
                    {data.slice(start, start + limit).map((item, index) => (
                        <li
                            className="services_item animate"
                            style={{ "--i": index }}
                            key={index}
                        >
